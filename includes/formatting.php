...
$sidebar_selection = of_get_option('widgeon_sidebars', 'right');
function widgeon_get_sidebar_selection(){
	// an object-oriented MVC would be a better solution... but this is far simpler.
	global $sidebar_selection;
	/*
	if($sidebar_selection == '' || !$sidebar_selection){
		$sidebar_selection = of_get_option('widgeon_sidebars', 'right');
	}
	*/
...
function widgeon_has_sidebar($classes) {
...
add_filter('body_class','widgeon_has_sidebar');
