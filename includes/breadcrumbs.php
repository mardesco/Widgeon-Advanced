...
Notes: As edited by Denzel on the Karma dev team, and by Jesse Smith of Mardesco.
*/
	...
	// for now:
		...
		if(!is_front_page() ){//&& !is_home()
			...
			if ( is_page() || is_single()) {	
				...
				if($output != ''){
					echo $markup . '<span class="current_crumb">';
					echo $output;
					echo ' </span>';
					}
			}
...
function simple_breadcrumb_case($der_post){
	...
	if (is_page()){
		 if($der_post->post_parent) {
			 ...
			 $link .= '">';
			 $link .= ''. get_the_title($my_query->ID) . '</a>';//. $markup; // including $markup in this line was causing duplicate spacers in some scenarios.  Will its removal cause an absence of spacers in other scenarios?  Keep watching to find out!
			 return $link;
		  }	
	return '';			 	
	} 
	...
	if(is_single()){
		...
		if (is_attachment()){
			...
			if (!empty($category)) { 
				...
				return '<a href="'.esc_url($catlink).'">'.esc_html($category[0]->cat_name).'</a>';// &nbsp;&gt;&nbsp; '.get_the_title();
				}else{
	...
	if( is_tag() ) { 
		$value = esc_html('Posts Tagged &quot;','wwd_breadcrumbs');
		$value .= esc_html( single_tag_title("", false), 'wwd_breadcrumbs' );
		$value .= esc_html('&quot;: ', 'wwd_breadcrumbs');
		return  $value;
		}
	...
	if(is_home()){ return esc_html(get_option( 'blogname', '') . ' Blog', 'wwd_breadcrumbs'); }
