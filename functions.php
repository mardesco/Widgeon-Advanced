...
add_action( 'optionsframework_custom_scripts', 'widgeon_toggle_slideshow_options' );

function widgeon_toggle_slideshow_options() { ?>

<script type="text/javascript">
jQuery(document).ready(function() {

	// slideshow 1
	jQuery('#widgeon_slideshow1_showhidden').click(function() {
  		jQuery('.widgeon-slideshow-1').fadeToggle(400);
	});

	if (jQuery('#widgeon_slideshow1_showhidden:checked').val() !== undefined) {
		jQuery('.widgeon-slideshow-1').show();
	}
	
	// slideshow 2
	jQuery('#widgeon_slideshow2_showhidden').click(function() {
  		jQuery('.widgeon-slideshow-2').fadeToggle(400);
	});

	if (jQuery('#widgeon_slideshow2_showhidden:checked').val() !== undefined) {
		jQuery('.widgeon-slideshow-2').show();
	}

	// slideshow 3
	jQuery('#widgeon_slideshow3_showhidden').click(function() {
  		jQuery('.widgeon-slideshow-3').fadeToggle(400);
	});

	if (jQuery('#widgeon_slideshow3_showhidden:checked').val() !== undefined) {
		jQuery('.widgeon-slideshow-3').show();
	}	
...
if ( ! isset( $content_width ) ){ 
	if(of_get_option('widgeon_max_width')){
		$content_width = (int) of_get_option('widgeon_max_width');
		}else{
			$content_width = 1024;
		}
	}
...
/*
arguably, custom meta boxes another PLUGIN TERRITORY TRESPASS
*/
...
// new! sliders to go with those meta boxes.
...
/*
and yet, Justin Tadlock's Stargazer theme has breadcrumbs included in theme functionality.
As of 3-10-14, I have tweeted @nacin and @Ipstenu requesting clarification.
*/
// and until I hear back from them...
...
function widgeon_color_scheme($classes){
	// get the user-selected color scheme. Defaults to grey.
	// other options: blue, green, or maroon.
	$color = esc_attr(of_get_option('widgeon_colors', 'grey'));
	if($color){
...
add_filter('body_class', 'widgeon_color_scheme');
